...
export function PostCard({ post, onUpdate }) {
  ...
  const pct = Math.min(100, Math.max(0, (ttl / maxTtl) * 100));
  const urgency = ttl > 7 ? 'green' : ttl > 4 ? 'yellow' : 'red';
  const opacity = Math.max(0.35, Math.min(1, ttl / 8));
  ...
  const handleLike = async (e) => {
    ...
    try {
      ...
      if (data.ok && onUpdate) onUpdate(data.posts);
    } finally {
      setLiking(false);
    }
  };
